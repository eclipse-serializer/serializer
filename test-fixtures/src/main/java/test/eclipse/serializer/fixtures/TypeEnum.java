...
public enum TypeEnum
{
    ...
    final private Supplier<? extends BinaryHandlerTestData> originalSupplier;
    final private Supplier<?> instanceSupplier;

    TypeEnum(Supplier<? extends BinaryHandlerTestData> original, Supplier<?> copy)
    {
        this.originalSupplier = original;
        this.instanceSupplier = copy;
    }

    /**
     * Returns a fresh, fully populated fixture instance on every call. Deliberately not a shared
     * singleton: these fixtures are persisted into separate storages by different tests, and a
     * shared instance (e.g. one holding a Lazy) would carry storage-bound state across tests,
