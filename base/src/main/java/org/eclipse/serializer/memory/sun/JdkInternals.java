...
public final class JdkInternals
{
	...
	public static final void ensureClassInitialized(final Class<?> c)
	{
		if(METHOD_MethodHandler_Lookup_ensureInitialized != null)
		{
			...
			catch(final Exception iae) // cannot catch IllegalAccessException because we use reflection
			{
				/*
				 * We have to fall back to Class.forName
				 * 
				 * In Java 22 Unsafe#ensureClassInitialized was removed.
				 * Since Java 15 there is MethodHandles.Lookup#ensureInitialized,
				 * but due to access restrictions we may not be able to call it for user classes.
				 * So, the only way to ensure class initialization at this stage is this one.
				 * 
				 * See https://bugs.openjdk.org/browse/JDK-8245871
				 */
				try
				{
