...
public interface BundleInfo
{
	...
	{
		return Load("serializer-base");  // FIXME This is not the bundle name but module name (file)
		// So it never matches the manifest entry, but BundleInfo is created within code based on pom.properties.
	}
		...
		public static class ManifestAndMavenBased implements Provider
		{
			...
			private Properties loadPomProperties() throws IOException
			{
				final URL pomPropertiesUrl = Thread.currentThread().getContextClassLoader().getResource(
					"META-INF/maven/org.eclipse.serializer/" + this.bundleName + "/pom.properties"
				);
