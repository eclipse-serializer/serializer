...
public class NativeLibraryJarLoader
{
	...
	private static String extractLibrary(URL libUrl)
	{
		...
		try (InputStream in = libUrl.openStream())
		{
			...
			logger.debug("Extracting library {} from jar to {} temp directory {}", libUrl.getFile(), library);
	...
	private static URL getJarLibraryURL()
	{
		...
		if(url == null)
		{
			...
			if(url == null)
			{
				logger.error("alternate library {} not found!", NATIVE_LIBRARY_FALLBACK_NAME);
			}
		}
		return url;
	}
